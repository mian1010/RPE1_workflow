...
workflow PreProcessingForVariantDiscovery_GATK4 {
  input {
    ...
    Float small_io_multiplier = 1.5
    Float medium_io_multiplier = 2.5
    Float large_io_multiplier = 4
  ...
  scatter (unmapped_bam in flowcell_unmapped_bams) {
    ...
    call SamToFastqAndBwaMem {
      input:
        ...
        bwa_path = gotc_path,
        gotc_path = gotc_path,
        io_multiplier = medium_io_multiplier,
        preemptible_tries = preemptible_tries,
  ...
  call SortAndFixTags {
    input:
      ...
      docker_image = gatk_docker,
      gatk_path = gatk_path,
      io_multiplier = large_io_multiplier,
      preemptible_tries = 0,
      compression_level = compression_level
  }
  ...
  scatter (subgroup in CreateSequenceGroupingTSV.sequence_grouping) {
    ...
    call BaseRecalibrator {
      input:
        ...
        docker_image = gatk_docker,
        gatk_path = gatk_path,
        io_multiplier = small_io_multiplier,
        preemptible_tries = preemptible_tries
    }  
  }  
...
task SamToFastqAndBwaMem {
  ...
  input {
    ...
    Float input_bam_size = size(input_bam, "GB") * io_multiplier
    Float ref_size = size(ref_fasta, "GB") + size(ref_fasta_index, "GB") + size(ref_dict, "GB") + size(ref_amb, "GB") + size(ref_ann, "GB") +  + size(ref_bwt, "GB") + size(ref_pac, "GB") + size(ref_sa, "GB") + if defined(ref_alt) then size(ref_alt, "GB") else 0
    Int disk_size = ceil(input_bam_size + ref_size) + 20
...
task MergeBamAlignment {
  input {
    ...
    Float input_bam_size = (size(unmapped_bam, "GB") + size(aligned_bam, "GB")) * io_multiplier
...
task SortAndFixTags {
  input {
    ...
    Int disk_size = ceil(input_bam_size + ref_size) + 100
...
task MarkDuplicates {
  input {
    ...
    Float input_bams_size = size(input_bams, "GB") * io_multiplier
    Int disk_size = ceil(input_bams_size) + 20
...
task BaseRecalibrator {
  ...
  Float input_bam_size = (size(input_bam, "GB") + size(input_bam_index, "GB")) * io_multiplier
  Float ref_size = size(ref_fasta, "GB") + size(ref_fasta_index, "GB") + size(ref_dict, "GB") + size(dbSNP_vcf, "GB") + size(dbSNP_vcf_index, "GB") + size(known_indels_sites_VCFs, "GB") + size(known_indels_sites_indices, "GB")
  Int disk_size = ceil(input_bam_size + ref_size) + 20
...
task GatherBqsrReports {
 input { 
  ...
  Float input_bqsr_size = size(input_bqsr_reports, "GB") * io_multiplier
  Int disk_size = ceil(input_bqsr_size) + 20
...
task ApplyBQSR {
  ...
  Float input_bam_size = (size(input_bam, "GB") + size(input_bam_index, "GB")) * io_multiplier
  Float ref_size = size(ref_fasta, "GB") + size(ref_fasta_index, "GB") + size(ref_dict, "GB") + size(recalibration_report, "GB")
  Int disk_size = ceil(input_bam_size + ref_size) + 20
...
task GatherBamFiles {
  ...
  Float input_bam_size = (size(input_bams, "GB")) * io_multiplier
  Int disk_size = ceil(input_bam_size) + 20
